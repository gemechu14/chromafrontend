...
function SetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await authApi.setPasswordFromInvitation(token, password);
      setSuccess(true);
    ...
    return (
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-card">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          This invitation link is missing a token. Ask your administrator to send a new invitation.
        </p>
        <Link href="/login" className="text-sm font-medium text-[#6C3FC5] hover:underline">
          Back to login
        </Link>
      </div>
    );
  ...
  if (success) {
    return (
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-card text-center">
        ...
        <h1 className="font-display text-xl font-bold text-foreground mb-2">Password set</h1>
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-card">
      <div className="flex items-center gap-2.5 mb-6">
        ...
        <div>
          <h1 className="font-display text-xl font-bold text-foreground">Set your password</h1>
          <p className="text-sm text-muted-foreground">Complete your Chroma account setup.</p>
        </div>
      </div>
